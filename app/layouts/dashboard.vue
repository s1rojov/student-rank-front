<template>
  <div
    class="min-h-screen bg-linear-to-br from-slate-50 via-blue-50 to-slate-100 overflow-x-hidden relative"
  >
    <aside
      :class="[
    ...
    >
      ...
      >
        ...
        >
          <p class="text-lg font-bold text-white tracking-tight truncate">
            StudentRank
          </p>
      ...
      <nav
        class="p-4 space-y-1 mt-2 h-[calc(100vh-220px)] custom-scrollbar overscroll-x-none"
      >
        <div class="space-y-2">
          <NuxtLink
          ...
          >
            ...
            <span v-if="!sidebarCollapsed" class="font-medium truncate">{{
              item.label
            }}</span>
            <span
              v-if="sidebarCollapsed"
              class="absolute left-full ml-4 px-2 py-1 bg-slate-900 text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50 shadow-xl border border-slate-700"
            >
              {{ item.label }}
            </span>
          </NuxtLink>
      ...
      <div
        class="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-700/50 bg-slate-900/80"
      >
        <button
          @click="handleLogout"
          :class="[
    ...
    >
      <header
        class="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-30 border-b border-gray-200 w-full"
      >
        <div class="px-4 md:px-8 py-4">
          <div class="flex items-center justify-between gap-4">
            <div class="flex items-center gap-4 min-w-0">
              ...
              <div class="min-w-0">
                <h1
                  class="text-lg md:text-2xl font-bold text-gray-900 truncate flex items-center gap-2"
                >
                  {{ pageTitle }}
                </h1>
              </div>
            ...
            <div class="flex items-center gap-2 shrink-0">
              <div class="relative hidden sm:block">
                <input
                  ...
                  class="pl-4 pr-4 py-2 border border-gray-200 rounded-xl text-sm w-32 md:w-64 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                />
              </div>
              <UButton
